<template>
  <div class="p-6 max-w-lg mx-auto bg-white shadow-xl rounded-xl">
    ...
    <section>
      <h2 class="text-xl font-semibold mb-4 border-b pb-2 text-gray-700">Tarefas Cadastradas</h2>
      
      <div v-if="isLoading" class="text-center text-gray-500">Carregando tarefas...</div>
      
      <div v-else-if="tasks.length === 0" class="text-center text-gray-500 p-4 border rounded-md">
      ...
      <ul class="space-y-3">
        <li
          ...
          :class="{'bg-green-50/50 border-green-200': task.completed, 'bg-gray-50': !task.completed}"
        >
          <div class="flex items-center space-x-3">
            ...
            />
            <span :class="{'line-through text-gray-500': task.completed, 'text-gray-800': !task.completed}">
              {{ task.title }}
            </span>
          </div>
